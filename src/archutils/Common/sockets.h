...
inline int nonblocking_recv(int socket, char* buffer, sockaddr_in* remaddr, size_t bufferSize)
{
#ifdef _WIN32
	...
	if (l > 0)
	{
		int result = recvfrom(socket, buffer, bufferSize, 0, (struct sockaddr*)&remaddr, &addrlen);
		if (result == -1)
		{
	...
	return recvfrom(socket, buffer, bufferSize, MSG_DONTWAIT, (struct sockaddr*)remaddr, &addrlen);
#endif /* _WIN32 */
